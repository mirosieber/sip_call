...
template <class T> struct remove_reference<const T &> {
  using type = T;
};
template <class T> struct remove_reference<T &&> {
  using type = T;
...
template <class, class...> struct is_unique;
...
template <class... Ts> using is_unique_t = is_unique<none_type, Ts...>;
...
template <int N, class T> T &get_by_id(tuple_type<N, T> *object) {
  return static_cast<tuple_type<N, T> &>(*object).value;
}
struct init {};
template <class T> struct pool_type {
...
template <class T> T &try_get(pool_type<T> *object) {
  return static_cast<pool_type<T> &>(*object).value;
}
template <class T, class TPool> T &get(TPool &p) {
  return static_cast<pool_type<T> &>(p).value;
}
template <class T, class TPool> const T &cget(TPool &p) {
...
template <> struct pool<> {
  ...
  template <class... Ts> explicit pool(Ts &&...) {}
  __BOOST_SML_ZERO_SIZE_ARRAY(byte);
};
template <class> false_type has_type(...);
template <class T> true_type has_type(const pool_type<T> *);
...
template <int... Ts> constexpr int max() { return max_impl(Ts...); }
#else
template <int... Ts> constexpr int max() {
  int max = 0;
...
struct zero_wrapper<TExpr, void_t<decltype(+declval<TExpr>())>>
  ...
  template <class... Ts> zero_wrapper(Ts &&...) {}
...
template <class T> struct string<T> {
  ...
  template <class U> static decltype(U::c_str()) c_str_impl(U *) {
    return U::c_str();
  }
...
template <class TException> struct exception : internal_event {
...
template <class T> struct sub_sm<sm_impl<sm_policy<T>>> {
  ...
  get(aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>>
          *object) {
...
template <class> no_policy get_policy(...);
template <class T, class TPolicy> TPolicy get_policy(aux::pair<T, TPolicy> *);
template <class, class...> struct sm_policy;
template <class SM, class... TPolicies> struct sm_policy {
  using sm = SM;
  using thread_safety_policy = decltype(get_policy<thread_safety_policy__>(
      (aux::inherit<TPolicies...> *)0));
  using defer_queue_policy = decltype(get_policy<defer_queue_policy__>(
      (aux::inherit<TPolicies...> *)0));
  using logger_policy =
...
void log_process_event(const aux::type<TLogger> &, TDeps &deps,
                       const TEvent &event) {
...
void log_state_change(const aux::type<no_policy> &, TDeps &, const TSrcState &,
                      const TDstState &) {}
template <class SM, class TLogger, class TDeps, class TSrcState,
          class TDstState>
void log_state_change(const aux::type<TLogger> &, TDeps &deps,
...
void log_action(const aux::type<no_policy> &, TDeps &, const TAction &,
                const TEvent &) {}
...
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
void log_action(const aux::type<TLogger> &, TDeps &deps,
...
void log_guard(const aux::type<no_policy> &, TDeps &, const TGuard &,
               const TEvent &, bool) {}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
void log_guard(const aux::type<TLogger> &, TDeps &deps, const TGuard &guard,
               const TEvent &event, bool result) {
  return static_cast<aux::pool_type<TLogger &> &>(deps)
      .value.template log_guard<SM>(guard, event, result);
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
void log_guard(const aux::type<TLogger> &, TDeps &deps,
...
template <> struct transitions<aux::false_type> {
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute(const TEvent &, SM &, TDeps &, TSubs &,
                      typename SM::state_t &) {
    return false;
  }
};
...
template <class TSM> struct transitions_sub<sm<TSM>> {
  ...
  static bool execute(const TEvent &event, SM &, TDeps &deps, TSubs &subs,
                      typename SM::state_t &) {
    sub_sm<sm_impl<TSM>>::get(&subs).template process_event<TEvent>(event, deps,
                                                                    subs);
    return true;
  }
};
...
struct mappings<aux::pool<Ts...>>
    : unique_mappings_t<
          event_mappings<typename Ts::event,
...
transitions<Ts...>
get_state_mapping_impl(state_mappings<T, aux::type_list<Ts...>> *);
...
template <class T, class... Ts>
transitions_sub<T, Ts...>
get_sub_state_mapping_impl(state_mappings<T, aux::type_list<Ts...>> *);
template <class T, class TMappings, class TUnexpected>
...
template <class T>
aux::false_type
test_callable(aux::non_type<void (callable_fallback::*)(), &T::operator()> *);
...
#endif
namespace back {
template <class TSM> struct sm_impl {
...
#if BOOST_SML_DISABLE_EXCEPTIONS
    const auto handled = process_event_impl<
  ...
  template <class TState> void initialize(const aux::type_list<TState> &) {
  ...
  template <class TDeps, class TSubs> void start(TDeps &deps, TSubs &subs) {
  ...
  void process_defer_events(TDeps &deps, TSubs &subs, const bool handled,
    ...
    if (handled) {
      ...
      using dispatch_table_t =
          bool (sm_impl::*)(TDeps &, TSubs &, const void *);
      const static dispatch_table_t
...
public:
  ...
  sm(aux::init, deps_t &deps, typename TSM::sm &sm)
      : deps_(deps), sub_sms_{aux::pool<typename TSM::sm &, deps_t>{sm, deps}} {
...
template <class TSM, class TDeps, class TSubs> struct sm_ref {
  template <class TEvent> auto process_event(const TEvent &event) {
    return aux::get<get_root_sm_t<TSubs>>(subs).process_event(event, deps,
                                                              subs);
  }
  TDeps &deps;
  TSubs &subs;
...
struct sm_ref_fwd {
  template <class TEvent> auto process_event(const TEvent &) {}
};
template <class, class, class> aux::type_list<action_base> args1__(...);
...
auto args__(int)
    -> aux::function_traits_t<decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD
                                       operator()<back::get_event_t<E>, TSM>)>;
template <class T, class E, class TSM>
...
decltype(auto) get_arg(const aux::type<T> &, const TEvent &, TSM &,
                       TDeps &deps) {
  return aux::get<T>(deps);
}
template <class TSM, class TEvent, class TDeps>
decltype(auto) get_arg(const aux::type<sm_ref_fwd &> &, const TEvent &, TSM &sm,
                       TDeps &) {
  return sm;
}
template <class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<TEvent> &, const TEvent &event, TSM &,
                       TDeps &) {
  return event;
}
template <class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<const TEvent &> &, const TEvent &event,
                       TSM &, TDeps &) {
  return event;
}
template <class T, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<const TEvent &> &,
                       const back::unexpected_event<T, TEvent> &event, TSM &,
                       TDeps &) {
  return event.event_;
}
template <class T, class TEvent, class TSM, class TDeps>
...
struct call<TEvent, aux::type_list<>, TLogger> {
  template <class T, class TSM, class TDeps, class TSubs>
  static auto execute(T object, const TEvent &event, TSM &, TDeps &deps,
                      TSubs &) {
...
struct call<TEvent, aux::type_list<Ts...>, TLogger> {
  ...
  static auto execute_impl(const aux::type<bool> &, T object,
                           const TEvent &event, SM &, TDeps &deps,
                           TSubs &subs) {
...
private:
  ...
  auto for_all(const aux::index_sequence<Ns...> &, const TEvent &event, TSM &sm,
    ...
    (void)aux::swallow{
        0,
        (result && call<TEvent, args_t<Ts, TEvent>,
...
public:
  ...
  template <class T> defer_event(T object) {
...
struct defer_queue : aux::pair<back::defer_queue_policy__, defer_queue<T>> {
  ...
  template <class... Ts> using defer = front::actions::defer_event<Ts...>;
};
template <class T> struct logger : aux::pair<back::logger_policy__, logger<T>> {
  using type = T;
};
...
template <class T, class TPolicy, class... TPolicies> struct rebind_policies {
...
struct process {
  ...
  template <class TEvent> auto operator()(const TEvent &event) {
...
template <class TState> struct state_impl {
  template <class T> auto operator<=(const T &t) const {
...
template <class TState> struct state : state_impl<state<TState>> {
  ...
  template <class... Ts> auto operator()(const state<Ts> &...) const {
    return state<TState(Ts...)>{};
  }
  template <class T> auto operator=(const T &t) const {
    return transition<T, state>{t, *this};
  }
  template <class T> auto sm() const {
...
struct transition<state<S2>, G, A>
  ...
  transition(const G &g, const A &a)
      : transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                   A>{g, a} {}
  template <class T> auto operator=(const T &) const {
...
void update_composite_states(TSubs &subs, aux::true_type,
...
void update_current_state(SM &, TDeps &deps, TSubs &,
                          typename SM::state_t &current_state,
...
void update_current_state(SM &, TDeps &deps, TSubs &subs,
                          typename SM::state_t &current_state,
                          const typename SM::state_t &new_state,
                          const TSrcState &, const state<back::sm<T>> &) {
  back::log_state_change<typename SM::sm_t>(
      aux::type<typename SM::logger_t>{}, deps,
...
#if !defined(_MSC_VER)
template <class T, T... Chrs> auto operator""_s() {
  return front::state<aux::string<T, Chrs...>>{};
}
template <class T, T... Chrs> auto operator""_e() {
...
#elif defined(__GNUC__)
// #undef __has_builtin  // Commented out to prevent breaking standard library
// headers

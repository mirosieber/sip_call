...
static void rtp_task(void *pvParameters) {
  ...
  // Initialize I2S and display once at start (optional hardware)
  static bool hardware_initialized = false;
  if (!hardware_initialized) {
    ESP_LOGI("RTP", "Hardware init skipped (no I2S/display connected)");
  ...
  for (;;) {
    if (!socket->is_initialized()) {
      ...
      ESP_LOGV("RTP", "socket not initialized");
      continue;
    }
    ...
    if (packet.size() > 0) {
      ...
      if ((int)packet[0] == 128) {
        ...
        // std::cout << "duration = " << statO.processingTime << " ms, freeRAM =
        // " << statO.freeRam << std::endl;
      ...
      if (statFull) {
        ...
        for (int k = 0; k < 100; k++) {
          std::cout << statistics[k].processingTime << ";"
                    << statistics[k].freeRam << std::endl;
        }
        istat = 0;
        statistics.clear();
        statFull = false;
...
template <class SocketT, class Md5T> class SipClientInt {
public:
  SipClientInt(const std::string &user, const std::string &pwd,
      ...
      : m_socket(server_ip, server_port, LOCAL_PORT)
        ...
        ,
        m_rtp_socket(server_ip, "7078", LOCAL_RTP_PORT), m_server_ip(server_ip),
        m_user(user), m_pwd(pwd), m_my_ip(my_ip), m_uri("sip:" + server_ip),
        m_to_uri("sip:" + user + "@" + server_ip),
        m_sip_sequence_number(std::rand() % 2147483647),
        m_call_id(std::rand() % 2147483647), m_response(""), m_realm(""),
        m_nonce(""), m_tag(std::rand() % 2147483647),
        m_branch(std::rand() % 2147483647), m_caller_display(m_user),
        m_sdp_session_id(0), m_command_event_group(xEventGroupCreate()) {
  ...
  bool is_initialized() const { return m_socket.is_initialized(); }
  ...
  void set_my_ip(const std::string &my_ip) { m_my_ip = my_ip; }
   ...
   *
   * \param[in] local_number A number that is registered locally on the server,
   * e.g. "**610"
   * \param[in] caller_display This string is displayed on the caller's phone
   */
  void request_ring(const std::string &local_number,
...
private:
  ...
  void tx() {
    ESP_LOGD(TAG, "tx() called, state=%d", (int)m_state);
    switch (m_state) {
    case SipState::IDLE:
      // fall-through
    case SipState::REGISTER_UNAUTH:
      // sending REGISTER without auth
      ESP_LOGI(TAG, "State: REGISTER_UNAUTH, sending initial REGISTER");
    ...
    case SipState::REGISTER_AUTH:
      ...
      ESP_LOGI(TAG, "State: REGISTER_AUTH, sending authenticated REGISTER");
    ...
    case SipState::RINGING:
      ...
      if (xEventGroupWaitBits(m_command_event_group, COMMAND_CANCEL_BIT, true,
                              true, 0)) {
  ...
  void rx() {
    if (m_state == SipState::REGISTERED) {
      if (xEventGroupWaitBits(m_command_event_group, COMMAND_DIAL_BIT, true,
                              true, 0)) {
    ...
    // The timeout is a workaround to be able to end a CANCEL request during
    // ring
    ...
    ESP_LOGI(TAG, "Received SIP packet: %d bytes", recv_string.size());
    ...
    ESP_LOGI(TAG, "Parsing the packet ok, reply code=%d",
             (int)packet.get_status());

    if (reply == SipPacket::Status::SERVER_ERROR_500) {
      log_state_transition(m_state, SipState::ERROR);
      m_state = SipState::ERROR;
      return;
    } else if ((reply == SipPacket::Status::UNAUTHORIZED_401) ||
               (reply == SipPacket::Status::PROXY_AUTH_REQ_407)) {
    ...
    case SipState::REGISTERED:
      if (packet.get_method() == SipPacket::Method::INVITE) {
        // received an invite, answered it already with ok, so new call is
        // established, because someone called us
    ...
    case SipState::INVITE_UNAUTH:
      if ((reply == SipPacket::Status::UNAUTHORIZED_401) ||
        ...
        m_sip_sequence_number++;
                 ...
                 (reply == SipPacket::Status::SESSION_PROGRESS_183)) {
    ...
    case SipState::INVITE_AUTH:
      if ((reply == SipPacket::Status::UNAUTHORIZED_401) ||
        ...
        m_state = SipState::ERROR;
                 ...
                 (reply == SipPacket::Status::SESSION_PROGRESS_183) ||
                 (reply == SipPacket::Status::TRYING_100)) {
    ...
    case SipState::RINGING:
      if (reply == SipPacket::Status::SESSION_PROGRESS_183) {
        ...
        if (m_event_handler) {
          m_event_handler(
              SipClientEvent{SipClientEvent::Event::CALL_CANCELLED});
        }
        ...
        ESP_LOGV(TAG, "Go back to send invite with auth...");
                 ...
                 (reply == SipPacket::Status::BUSY_HERE_486)) {
        send_sip_ack();
        m_sip_sequence_number++;
        m_branch = std::rand() % 2147483647;
        m_state = SipState::REGISTERED;
        SipClientEvent::CancelReason cancel_reason =
            SipClientEvent::CancelReason::CALL_DECLINED;
      ...
      if (packet.get_method() == SipPacket::Method::BYE) {
        ...
        if (m_event_handler) {
          m_event_handler(SipClientEvent{SipClientEvent::Event::CALL_END});
                 ...
                 (packet.get_content_type() ==
                  SipPacket::ContentType::APPLICATION_DTMF_RELAY)) {
        if (m_event_handler) {
          m_event_handler(SipClientEvent{SipClientEvent::Event::BUTTON_PRESS,
                                         packet.get_dtmf_signal(),
                                         packet.get_dtmf_duration()});
        }
      }
      break;
    case SipState::CANCELLED:
  ...
  void send_sip_register() {
    ESP_LOGI(TAG, "Sending REGISTER (state=%d, has_auth=%s)", (int)m_state,
             m_response.empty() ? "no" : "yes");
    TxBufferT &tx_buffer = m_socket.get_new_tx_buf();
    std::string uri = "sip:" + m_server_ip;

    send_sip_header("REGISTER", uri, "sip:" + m_user + "@" + m_server_ip,
                    tx_buffer);

    tx_buffer << "Contact: \"" << m_user << "\" <sip:" << m_user << "@"
              << m_my_ip << ":" << LOCAL_PORT
              << ";transport=" << TRANSPORT_LOWER << ">\r\n";

    if (!m_response.empty()) {
      tx_buffer << "Authorization: Digest username=\"" << m_user
                << "\", realm=\"" << m_realm << "\", nonce=\"" << m_nonce
                << "\", uri=\"" << uri << "\", algorithm=MD5, response=\""
                << m_response << "\"\r\n";
    }
    tx_buffer << "Allow: INVITE, ACK, CANCEL, OPTIONS, BYE, REFER, NOTIFY, "
  ...
  void send_sip_invite() {
    ...
    if (!m_response.empty()) {
      // tx_buffer << "Proxy-Authorization: Digest username=\"" << m_user <<
      // "\", realm=\"" << m_realm << "\", nonce=\"" << m_nonce << "\", uri=\""
      // << m_uri << "\", response=\"" << m_response << "\"\r\n";
      tx_buffer << "Authorization: Digest username=\"" << m_user
                << "\", realm=\"" << m_realm << "\", nonce=\"" << m_nonce
                << "\", uri=\"" << m_uri << "\", response=\"" << m_response
                << "\"\r\n";
    }
    ...
    m_tx_sdp_buffer << "v=0\r\n"
                    << "o=" << m_user << " " << m_sdp_session_id << " "
                    << m_sdp_session_id << " IN IP4 " << m_my_ip << "\r\n"
                    << "s=sip-client/0.0.1\r\n"
                    << "c=IN IP4 " << m_my_ip << "\r\n"
                    << "t=0 0\r\n"
  ...
  void send_sip_header(const std::string &command, const std::string &uri,
                       const std::string &to_uri, TxBufferT &stream) {
    ...
    if (command == "REGISTER") {
      stream << "From: <sip:" << m_user << "@" << m_server_ip
             << ">;tag=" << m_tag << "\r\n";
    } else if (command == "INVITE") {
      stream << "From: \"" << m_caller_display << "\" <sip:" << m_user << "@"
             << m_server_ip << ">;tag=" << m_tag << "\r\n";
    } else {
      stream << "From: \"" << m_user << "\" <sip:" << m_user << "@"
             << m_server_ip << ">;tag=" << m_tag << "\r\n";
    }
    stream << "Via: SIP/2.0/" << TRANSPORT_UPPER << " " << m_my_ip << ":"
           << LOCAL_PORT << ";branch=z9hG4bK-" << m_branch << ";rport\r\n";
  ...
  void send_sip_reply_header(const std::string &code, const SipPacket &packet,
                             TxBufferT &stream) {
    stream << "SIP/2.0 " << code << "\r\n";
  ...
  bool read_param(const std::string &line, const std::string &param_name,
                  std::string &output) {
  ...
  void compute_auth_response(const std::string &method,
...
template <class SipClientT> struct sip_states {
...
template <class SocketT, class Md5T> class SipClient {
public:
  SipClient(const std::string &user, const std::string &pwd,
            const std::string &server_ip, const std::string &server_port,
            const std::string &my_ip)
  ...
  bool init() { return m_sip.init(); }

  bool is_initialized() const { return m_sip.is_initialized(); }

  void set_server_ip(const std::string &server_ip) {
    m_sip.set_server_ip(server_ip);
  }

  void set_my_ip(const std::string &my_ip) { m_sip.set_my_ip(my_ip); }
  ...
  void request_cancel() { m_sip.request_cancel(); }

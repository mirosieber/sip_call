...
public:
  ...
  Buffer<SIZE> &operator<<(const char *str) {
    strncat(m_buffer.data(), str,
            m_buffer.size() - strlen(m_buffer.data()) - 1);
    return *this;
  }
  Buffer<SIZE> &operator<<(const std::string &str) {
    strncat(m_buffer.data(), str.c_str(),
            m_buffer.size() - strlen(m_buffer.data()) - 1);
    return *this;
  }
  Buffer<SIZE> &operator<<(int8_t i) {
    snprintf(m_buffer.data() + strlen(m_buffer.data()),
             m_buffer.size() - strlen(m_buffer.data()), "%c", i);
    return *this;
  }
  ...
  Buffer<SIZE> &operator<<(uint16_t i) {
    snprintf(m_buffer.data() + strlen(m_buffer.data()),
             m_buffer.size() - strlen(m_buffer.data()), "%d", i);
    return *this;
  }
  Buffer<SIZE> &operator<<(uint32_t i) {
    snprintf(m_buffer.data() + strlen(m_buffer.data()),
             m_buffer.size() - strlen(m_buffer.data()), "%lu",
             (unsigned long)i);
    return *this;
  }
  Buffer<SIZE> &operator<<(size_t i) {
    snprintf(m_buffer.data() + strlen(m_buffer.data()),
             m_buffer.size() - strlen(m_buffer.data()), "%zu", i);
    return *this;
  }

  const char *data() const { return m_buffer.data(); }

  size_t size() const { return strlen(m_buffer.data()); }
...
class LwipUdpClient {
public:
  LwipUdpClient(const std::string &server_ip, const std::string &server_port,
                uint16_t local_port)
      : m_server_port(server_port), m_server_ip(server_ip),
        m_local_port(local_port), m_socket(INVALID_SOCKET) {
  ...
  bool init() {
    ...
    struct addrinfo *res;
    struct in_addr *addr;
    ESP_LOGI(TAG, "Doing DNS lookup for host=%s port=%s", m_server_ip.c_str(),
             m_server_port.c_str());

    int err =
        getaddrinfo(m_server_ip.c_str(), m_server_port.c_str(), &hints, &res);
  ...
  bool is_initialized() const { return m_socket >= 0; }
  ...
  std::string receive(uint32_t timeout_msec) {
    ...
    int readable =
        select(m_socket + 1, &m_rx_fds, nullptr, nullptr, &m_rx_timeval);
  ...
  bool send_buffered_data() {
    ...
    ssize_t result =
        sendto(m_socket, m_tx_buffer.data(), m_tx_buffer.size(), 0,
